...
//
// 注意：不再使用固定的 Context 字段，而是在每个方法调用时传递 context
// 这样可以支持请求级别的超时控制和链路追踪
...
func NewClient(address string, username string, password string, db int) *RedisClient {
    ...
    rds.Client = redis.NewClient(&redis.Options{
        ...
        DialTimeout:     5 * time.Second,  // 连接超时 5 秒
        ReadTimeout:     3 * time.Second,  // 读超时 3 秒
        WriteTimeout:    3 * time.Second,  // 写超时 3 秒
    })
...
func (rds RedisClient) Ping(ctx context.Context) error {
...
func (rds RedisClient) Has(ctx context.Context, key string) bool {
    _, err := rds.Client.Get(ctx, key).Result()
    if err != nil {
        if err != redis.Nil {
...
func (rds RedisClient) Del(ctx context.Context, keys ...string) bool {
...
func (rds RedisClient) FlushDB(ctx context.Context) bool {
    if err := rds.Client.FlushDB(ctx).Err(); err != nil {
...
func (rds RedisClient) Increment(ctx context.Context, parameters ...interface{}) bool {
...
func (rds RedisClient) Decrement(ctx context.Context, parameters ...interface{}) bool {
    switch len(parameters) {
    case 1:
        key := parameters[0].(string)
        if err := rds.Client.Decr(ctx, key).Err(); err != nil {
    ...
    case 2:
        key := parameters[0].(string)
        value := parameters[1].(int64)
        if err := rds.Client.DecrBy(ctx, key, value).Err(); err != nil {
            logger.ErrorString("Redis", "Decrement", err.Error())
            return false
        }
    default:
