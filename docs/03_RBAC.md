# 🔐 权限管理系统 (RBAC)

**最后更新**: 2026年1月1日 | **版本**: v2.0

---

## 📖 目录

1. [RBAC概念](#rbac概念)
2. [权限设计](#权限设计)
3. [角色定义](#角色定义)
4. [权限检查](#权限检查)
5. [最佳实践](#最佳实践)

---

## 🎯 RBAC概念

### 什么是RBAC?

**Role-Based Access Control** (基于角色的访问控制)

```
User ──belongs to──> Role ──has──> Permission

例如:
张三 (User) ──belongs to──> 版主 (Role) ──has──> 删除评论 (Permission)
```

### 权限模型关系

```
┌─────────────────────────────────────┐
│          User (用户)                │
├─────────────────────────────────────┤
│ id, name, email, ...                │
└─────────────┬───────────────────────┘
              │ 一对多
              ↓
┌──────────────────────────────────────┐
│      UserRole (用户-角色关联)         │
├──────────────────────────────────────┤
│ user_id, role_id, assigned_at       │
└─────────┬────────────────────────────┘
          │ 多对一
          ↓
┌──────────────────────────────────────┐
│         Role (角色)                  │
├──────────────────────────────────────┤
│ id, name, description, ...          │
└──────────┬─────────────────────────┘
           │ 一对多
           ↓
┌───────────────────────────────────────┐
│    RolePermission (角色-权限关联)      │
├───────────────────────────────────────┤
│ role_id, permission_id, granted_at  │
└──────────┬────────────────────────────┘
           │ 多对一
           ↓
┌───────────────────────────────────────┐
│      Permission (权限)                │
├───────────────────────────────────────┤
│ id, name, description, ...          │
└───────────────────────────────────────┘
```

---

## 📋 权限设计

### 权限命名规范

```
resource.action

示例:
- topics.create       创建话题
- topics.read         查看话题
- topics.update       编辑话题
- topics.delete       删除话题
- comments.create     发表评论
- comments.delete     删除评论
- admin.users         管理用户
- admin.roles         管理角色
```

### 完整权限列表

| 权限 | 描述 | 范围 |
|------|------|------|
| **topics.create** | 创建新话题 | 认证用户 |
| **topics.read** | 查看话题 | 公开 |
| **topics.update** | 编辑话题 | 作者或管理员 |
| **topics.delete** | 删除话题 | 作者或管理员 |
| **comments.create** | 发表评论 | 认证用户 |
| **comments.update** | 编辑评论 | 作者或管理员 |
| **comments.delete** | 删除评论 | 作者或管理员 |
| **admin.users** | 管理用户 | 管理员 |
| **admin.topics** | 管理话题 | 管理员 |
| **admin.comments** | 管理评论 | 管理员 |
| **admin.roles** | 管理角色 | 超级管理员 |
| **admin.permissions** | 管理权限 | 超级管理员 |

---

## 👤 角色定义

### 预设角色

| 角色 | 权限 | 说明 |
|------|------|------|
| **Guest** | 公开权限 | 游客（未登录） |
| **User** | topics.create<br>comments.create<br>等基础权限 | 普通用户 |
| **Moderator** | topics.delete<br>comments.delete<br>用户管理权限 | 版主 |
| **Admin** | 除权限管理外所有权限 | 管理员 |
| **SuperAdmin** | 全部权限 | 超级管理员 |

### 角色权限矩阵

```
                创建 查看 编辑 删除 管理
          话题   ✓   ✓   ⚠   ⚠   ✗
User      评论   ✓   ✓   ⚠   ⚠   ✗
          设置   ✓   ✓   ✓   ✗   ✗

          话题   ✓   ✓   ✓   ✓   ✓
Moderator 评论   ✓   ✓   ✓   ✓   ✓
          用户   ✗   ✓   ✗   ✗   ⚠

          话题   ✓   ✓   ✓   ✓   ✓
Admin     评论   ✓   ✓   ✓   ✓   ✓
          角色   ✗   ✓   ✗   ✗   ✗

Super     所有   ✓   ✓   ✓   ✓   ✓
Admin     权限   ✓   ✓   ✓   ✓   ✓

⚠ = 条件权限（仅限自己）
```

---

## 🔍 权限检查

### 方法1: 注解检查

```go
// @Authorize("topics.create")
func (uc *TopicController) Create(c *gin.Context) {
    // 自动检查权限
}
```

### 方法2: 中间件检查

```go
func RequirePermission(permission string) gin.HandlerFunc {
    return func(c *gin.Context) {
        userID := c.GetInt64("user_id")
        
        if !permissionService.HasPermission(userID, permission) {
            response.Error(c, "权限不足")
            c.Abort()
            return
        }
        
        c.Next()
    }
}

// 路由使用
router.POST("/api/topics", 
    RequirePermission("topics.create"), 
    topicController.Create)
```

### 方法3: 服务层检查

```go
func (s *TopicService) Create(ctx context.Context, req *CreateTopicRequest) error {
    userID := ctx.Value("user_id").(int64)
    
    // 检查权限
    if !s.permissionService.HasPermission(userID, "topics.create") {
        return errors.New("无创建话题的权限")
    }
    
    // 业务逻辑
    return s.repo.Create(ctx, &Topic{...})
}
```

### 方法4: 策略类检查

```go
type TopicPolicy struct {
    permissionService *PermissionService
}

// 检查用户是否能编辑话题
func (p *TopicPolicy) CanUpdate(userID int64, topic *Topic) bool {
    // 作者可以编辑
    if topic.UserID == userID {
        return true
    }
    
    // 管理员可以编辑
    return p.permissionService.HasPermission(userID, "admin.topics")
}

// 控制器使用
if !topicPolicy.CanUpdate(userID, topic) {
    return errors.New("无权限编辑此话题")
}
```

---

## ✨ 最佳实践

### 权限设计原则

1. **最小权限原则**
   - 只授予必要的权限
   - 定期审查和撤销过期权限

2. **职责分离**
   - 敏感操作需要多个权限
   - 例如: 删除用户需要 `admin.users` + `audit.sensitive`

3. **权限分组**
   - 按资源分组权限
   - 按操作分组权限

4. **审计跟踪**
   - 记录所有权限变更
   - 记录敏感操作

### 权限检查清单

```
在每个需要权限控制的接口:

✓ 在路由层添加权限检查中间件
✓ 在服务层重复验证权限
✓ 记录权限检查日志
✓ 处理权限不足的错误
✓ 添加单元测试验证权限
✓ 文档标注权限要求
```

### 常见陷阱

```
❌ 只在前端检查权限
✓ 必须在后端验证

❌ 信任用户提供的role_id
✓ 从Token或Session获取用户权限

❌ 权限检查不一致
✓ 使用统一的权限检查机制

❌ 隐式权限假设
✓ 明确检查需要的每个权限
```

---
