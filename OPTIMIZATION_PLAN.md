...
具体做法：
1. 按业务（如用户、话题、分类、链接等）将路由拆分到 routes/user.go、routes/topic.go 等独立文件。
2. 每个路由文件只注册本业务相关的路由及中间件，主 routes/api.go 只负责聚合调用。
3. 便于多人协作和后期维护，避免 api.go 文件过大难以管理。
4. 推荐结构示例：
   - routes/
     - api.go（主入口，聚合各模块路由）
     - user.go
     ...
     - category.go
     - link.go
5. 这样可提升可读性、可维护性，并支持后续业务扩展。
中间件注册顺序应关注安全（如 CORS、XSS、SecureHeaders）优先。
控制器与服务层
控制器只做参数校验与响应，业务逻辑全部下沉到 service 层。
service 层方法应返回详细错误，便于日志与追踪。
数据库与模型
