...
// Follow 关注用户
func (ctrl *UsersController) Follow(c *gin.Context) {
	targetUserID := c.Param("id")
	currentUserID := auth.CurrentUID(c)

	if err := ctrl.interactionService.FollowUser(currentUserID, targetUserID); err != nil {
		logger.LogErrorWithContext(c, err, "关注用户失败")
		response.ApiError(c, 500, err.Code, err.Message)
		return
	}
...
// Unfollow 取消关注
func (ctrl *UsersController) Unfollow(c *gin.Context) {
	targetUserID := c.Param("id")
	currentUserID := auth.CurrentUID(c)

	if err := ctrl.interactionService.UnfollowUser(currentUserID, targetUserID); err != nil {
		logger.LogErrorWithContext(c, err, "取消关注失败")
		response.ApiError(c, 500, err.Code, err.Message)
		return
	}
